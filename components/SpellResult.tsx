...
const getIcon = (system: MagicSystem, sizeClass: string = "w-4 h-4") => {
  switch (system) {
...
const DataRow: React.FC<{ label: string; value: string; full?: boolean }> = ({ label, value, full }) => (
  <div className={`p-2 bg-black/40 border border-white/5 rounded ${full ? 'col-span-2' : ''}`}>
    ...
    <div className="text-xs text-gray-300 font-sans leading-snug">{value}</div>
  </div>
);
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between text-xs bg-black/40 p-3 rounded border border-white/5">
                            <div className="text-gray-500 text-center flex-1">
                                ...
                                <div>{protection.lore.hierarchy.lower}</div>
                            </div>
                            <div className="text-gray-600 px-2">→</div>
                            <div className="text-white font-bold text-center flex-1 border px-2 py-1 rounded border-magic-accent/30 bg-magic-accent/10">
                                <div className="text-[8px] uppercase text-magic-accent">Current</div>
                                <div>{protection.lore.hierarchy.middle || protection.name}</div>
                            </div>
                            <div className="text-gray-600 px-2">→</div>
                            <div className="text-yellow-200 text-center flex-1">
                                <div className="text-[8px] uppercase">Upper</div>
                                <div>{protection.lore.hierarchy.upper}</div>
                            </div>
                        </div>
                    </div>
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-2 md:p-4">
      ...
      <div className="bg-[#0a0a0f] border border-white/10 rounded-sm overflow-hidden shadow-2xl relative ring-1 ring-white/5 backdrop-blur-3xl animate-in fade-in zoom-in-95 duration-500">
        ...
        {/* Holographic Header */}
        <div className="bg-white/5 border-b border-white/10 p-2 px-4 flex justify-between items-center">
             <div className="flex items-center gap-2 text-[10px] font-mono text-magic-accent tracking-widest">
                <Database className="w-3 h-3" />
                <span>ARCHIVE :: ENTRY {spell.id}</span>
             </div>
        ...
        <div className="grid lg:grid-cols-12 gap-0">
          ...
          <div className="lg:col-span-5 relative border-r border-white/10 flex flex-col items-center justify-center bg-black/20 min-h-[300px] lg:min-h-auto overflow-hidden">
            ...
            <div className="absolute bottom-0 w-full p-4 bg-black/40 border-t border-white/10 grid grid-cols-2 gap-2 backdrop-blur-sm">
                 <div className="p-2 border border-white/10 bg-white/5 rounded">
                    <div className="text-[8px] text-gray-500 font-mono uppercase">OIP 振幅 (Amplitude)</div>
                    <div className="text-xs text-magic-accent font-mono">{spell.oipAmplitude}</div>
                 </div>
                 <div className="p-2 border border-white/10 bg-white/5 rounded">
                    <div className="text-[8px] text-gray-500 font-mono uppercase">OIP 周波数 (Freq)</div>
                 ...
                 <div className="col-span-2 p-2 border border-white/10 bg-white/5 rounded flex items-center gap-2">
                    <Eye className="w-3 h-3" style={{ color: spell.eyeColor }} />
                    <div className="text-[9px] text-gray-400 font-mono">W視核共鳴: <span style={{ color: spell.eyeColor }}>Active</span></div>
                 </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 p-6 flex flex-col gap-6 bg-gradient-to-br from-[#0a0a0f] to-[#11111a]">
            ...
            <div>
               ...
               <h1 className="text-3xl md:text-4xl font-serif text-white magic-glow leading-tight mb-2 tracking-wide">{spell.name}</h1>
            ...
            <div className="grid grid-cols-3 gap-2 bg-white/5 p-3 rounded border border-white/5">
                <div>
                    <div className="text-[9px] text-gray-500 font-mono uppercase">系 (System)</div>
                    <div className="text-xs text-white flex items-center gap-1">{getIcon(spell.system)} {spell.system}</div>
                </div>
                <div>
                    <div className="text-[9px] text-gray-500 font-mono uppercase">属 (Genus)</div>
                    <div className="text-xs text-white">{spell.attribute}</div>
                </div>
                <div>
                    <div className="text-[9px] text-gray-500 font-mono uppercase">環 (Ring)</div>
                    <div className="text-xs text-white">Rank {spell.rank}</div>
                </div>
            </div>
            ...
            <div className="space-y-2">
               <div className="flex items-center gap-2 text-xs font-mono text-gray-500 uppercase tracking-widest border-b border-white/10 pb-1">
                  <Cpu className="w-3 h-3" /> 特徴・制約 (Features & Constraints)
               </div>
            ...
            <div className="space-y-1">
                ...
                <div className="p-3 bg-black/40 border-l-2 border-magic-accent text-sm italic text-gray-300 font-serif">
                    "{spell.chantFeedback}"
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400">
                <span className="font-mono uppercase text-gray-600">Famous User:</span> {spell.lore.famousUser}
            </div>

            {/* Influence & Damage (Collapsed by default logic or keep as is) */}
            <div className="pt-4 border-t border-white/10 grid md:grid-cols-2 gap-4">
               {/* Equipment Influence */}
               <div className="space-y-2 col-span-2">
                   <div className="flex items-center gap-2 text-xs font-mono text-gray-500 uppercase tracking-widest border-b border-white/10 pb-1">
                      <Shield className="w-3 h-3 text-yellow-500" /> Equipment & Blessings
                   </div>
                   <div className="grid grid-cols-2 gap-2">
                      <div className="p-2 bg-black/40 border border-white/5 rounded flex justify-between items-center group">
                         <div>
                            <div className="text-[9px] text-gray-500 font-mono uppercase mb-0.5 flex items-center gap-1">
                                <Shield className="w-3 h-3 text-magic-accent" /> Protection
                            </div>
                            <div className="text-xs text-gray-300 font-sans">{spell.protection.name}</div>
                         </div>
                         {spell.protection.id !== 'none' && (
                             <button 
                                onClick={() => setShowProtectionDetails(true)}
                                className="px-2 py-1 bg-white/10 hover:bg-magic-accent/20 border border-white/10 rounded text-[9px] text-gray-300 hover:text-white transition-colors flex items-center gap-1"
                             >
                      ...
                      <div className="p-2 bg-black/40 border border-white/5 rounded">
                         <div className="text-[9px] text-gray-500 font-mono uppercase mb-0.5 flex items-center gap-1">
                             <Hammer className="w-3 h-3 text-blue-400" /> Tool
                         </div>
                         <div className="text-xs text-gray-300 font-sans">{spell.tool.name} <span className="text-blue-500 font-mono text-[10px]">x{spell.tool.powerBonus}</span></div>
                      </div>
                   </div>
               </div>
            </div>
            ...
            <div className="space-y-2 mt-2">
                ...
                >
                    <div className="flex items-center gap-2 text-red-400/80 uppercase font-mono text-xs tracking-widest">
                        <Swords className="w-4 h-4" />
                        Predicted Damage
                        {showFormula ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3 opacity-50 group-hover:opacity-100" />}
                    </div>
                    <div className="text-2xl font-mono font-bold text-red-200 drop-shadow-[0_0_10px_rgba(220,38,38,0.5)]">
                ...
                {showFormula && spell.calculationFormula && (
                    <div className="p-3 bg-black/60 border border-white/10 rounded text-[10px] md:text-xs font-mono text-gray-400 animate-in slide-in-from-top-2">
                        ...
                        <pre className="whitespace-pre-wrap font-mono leading-relaxed">{spell.calculationFormula}</pre>
                    </div>
                )}
            </div>
